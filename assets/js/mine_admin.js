...
$(".saveEditCategorie").click(function() {
  ...
  $.ajax({
    ...
    data: {
      for_id: forIdEditCategorie,
      abbr: abbrEditCategorie,
      type: "shop_categorie",
      name: newValueFromEdit
    }
  }).done(function(data) {
...
function changeTextualPageStatus(id) {
  ...
  $.post(urls.changeTextualPageStatus, { id: id, status: status }, function(
    data
  ) {
    if (data == "1") {
      return true;
    ...
    function(data) {
      if (data == "1") {
        ...
        if (to_status == 1) {
          ...
          $('[data-action-id="' + id + '"]')
            .removeClass()
            .addClass("bg-success  text-center");
        }
        if (to_status == 2) {
  ...
  $.ajax({
    ...
    if (data == "1") {
      if (to_status == 1) {
        ...
        $('[data-article-id="' + id + '"] .status-is-icon').html(
...
$(".form-control.form_datetime").datetimepicker({
  format: "dd MM yyyy",
  weekStart: 1,
  todayBtn: 1,
  autoclose: 1,
